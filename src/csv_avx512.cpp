              ...
              int &numVals, int &val_type)
{
    ...
    int i;
    #pragma omp parallel private(i) reduction(merge \
                                          : value_hashmap)
    {
          ...
          const int &numRows, const int &numVal, const floatType *x, floatType *y)
{
    if (numVal <= 256)
    {
        ...
        #pragma omp parallel private(_col_ptr, _val_ptr, _val_ptr_uint8, _x, _y, _val, _mask, start, padding_end, end, row, i)
        {
            #pragma omp for schedule(static) nowait
            for (row = 0; row < numRows; row++)
            {
                ...
                _col_ptr = _mm256_maskz_load_epi32(_mask, &h_cols[end]);
    ...
    else if (numVal <= 65536)
    {
        ...
        #pragma omp parallel private(_col_ptr, _val_ptr, _val_ptr_uint16, _x, _y, _val, _mask, start, padding_end, end, row, i)
        {
            #pragma omp for schedule(static) nowait
            for (row = 0; row < numRows; row++)
            {
    ...
    else
    {
        ...
        #pragma omp parallel private(_col_ptr, _val_ptr, _x, _y, _val, _mask, start, padding_end, end, row, i)
        {
            #pragma omp for schedule(static) nowait
            for (row = 0; row < numRows; row++)
            {
